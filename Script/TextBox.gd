...
var text_queue = [
	"Pay EXP and get power !!! then click left mouse button to fire !!",
	"Remember his name is Scepter of the Sun !"
]
	...
	match currant_state:
		State.READY:
			if !text_queue.empty():
				display_text()
		State.READING:
			if Input.is_action_just_pressed("ui_accept") or Input.is_action_just_pressed("ui_pick"):
		...
		State.FINISH:
			if Input.is_action_just_pressed("ui_accept") or Input.is_action_just_pressed("ui_pick"):
				...
				hide_textbox()
			if Input.is_action_just_pressed("ui_mouse_left") or Input.is_action_just_pressed("ui_pick"):
				change_state(State.READY)
				hide_textbox()
